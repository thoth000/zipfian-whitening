...
class STSEval(object):
    ...
    def run(self, params, batcher):
        ...
        all_sys_scores = []
        all_gs_scores = []
        # XXX: precompute the whitening matrix for the whole dataset for a task, including subtasks
        if (self.whitening_calculator is not None) and (
            self.whitening_calculator.whitening_class is not None
        ):
            logging.debug(
                f"Pre computing whitening matrix for {self.whitening_calculator.whitening_class}"
            )

            for dataset in self.datasets:
                logging.debug(f"calculating hidden states for {dataset}")
                ...
                for ii in range(0, len(gs_scores), params.batch_size):
                    ...
                    if len(batch1) == len(batch2) and len(batch1) > 0:
                        ...
                        for i in range(len(batch1)):
                            batch.extend([batch1[i], batch2[i]])
                        self.whitening_calculator.compute_and_store_hidden_states(batch)

            logging.debug(f"Concatenating hidden states for all subsets")
            self.whitening_calculator.concatenate_hidden_states()
            logging.debug(
                f"Computed token embeddings: {self.whitening_calculator.filtered_first_hidden.shape}"
            )
            logging.debug(f"Computing whitening matrix...")
            self.whitening_calculator.compute_whitening_matrix()

        logging.debug("Starting the evaluation")
        for dataset in self.datasets:
            sys_scores = []
            input1, input2, gs_scores = self.data[dataset]
            for ii in range(0, len(gs_scores), params.batch_size):
                ...
                # we assume get_batch already throws out the faulty ones
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    # XXX: To apply the same linear transformation to both sentences, merge two batches and apply the transformation once
                    batch = []
                    for i in range(len(batch1)):
                        batch.extend([batch1[i], batch2[i]])
                    enc = batcher(
                        params, batch, whitening_calculator=self.whitening_calculator
                    )
                    for kk in range(enc.shape[0] // 2):
                        sys_score = self.similarity(enc[2 * kk], enc[2 * kk + 1])
...
class STS12Eval(STSEval):
    def __init__(self, taskpath, seed=1111, whitening_calculator=None):
        ...
        self.whitening_calculator = whitening_calculator
...
class STSBenchmarkEval(STSEval):
    def __init__(self, task_path, seed=1111, whitening_calculator=None):
